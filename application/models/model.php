...
class Model extends CI_Model {
    ...
    function get_rekom($tabel,$urut="asc") {
        $this->db->from($tabel);
         $this->db->order_by("pringkat", $urut); 
        $query = $this->db->get(); //cek apakah ada ba 
    ...
    $query = $this->db->get('perbandingan_kriteria');
    if($query->num_rows()>0)
    {
    ...
    function join2table(){
        ...
        $this->db->order_by('pringkat');
        $this->db->limit(50 ,"DESC");
        // $this->db->where ('peringkat', $antara);
        $this->db->join('calon_siswa','calon_siswa.id = hasil_rekomendasi.id');      
        $query = $this->db->get();
        return $query;
     }
     ...
     function getdatarangkingawal($tahun=null)
    {
      ...
      LEFT JOIN hasil_rekomendasi h ON h.tahun =  a.tahun AND h.nis = a.nis
      where $where
     ...
     function gethasilpenentuan($tahun=2019)
    {
